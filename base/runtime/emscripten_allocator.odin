...
emscripten_allocator :: proc "contextless" () -> Allocator {
    return Allocator{emscripten_allocator_proc, nil}
}
...
// cannot use maps and simd features.
    ...
    aligned_alloc :: proc(size, alignment: int, zero_memory: bool, old_ptr: rawptr = nil) -> ([]byte, Allocator_Error) {
        ...
        slice := byte_slice(aligned_mem, size)
    ...
    aligned_free :: proc(p: rawptr) {
        if p != nil {
            free(ptr_offset((^rawptr)(p), -1)^)
        }
    }

    aligned_resize :: proc(p: rawptr, old_size: int, new_size: int, new_alignment: int) -> ([]byte, Allocator_Error) {
        if p == nil {
            return nil, nil
        }
